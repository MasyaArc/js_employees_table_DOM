...
document.addEventListener("click", (e)=>{
    ...
    if (tH.classList.contains("ASC")) {
        rows.sort((rowA, rowB)=>{
            ...
            if (columnIndex === 0 || columnIndex === 1 || columnIndex === 2) return valueB.localeCompare(valueA);
            if (columnIndex === 4) {
    ...
    rows.sort((rowA, rowB)=>{
        ...
        if (columnIndex === 0 || columnIndex === 1 || columnIndex === 2) return valA.localeCompare(valB);
        if (columnIndex === 4) {
...
document.addEventListener("click", (e)=>{
    ...
    for(let i = 0; i < rows.length; i++)if (rows[i].classList.contains("active")) rows[i].classList.remove("active");
...
function addNewPersonTable() {
    ...
    form.addEventListener("submit", (e)=>{
        ...
        if (form.elements.age.value.length === 0 || Number(form.elements.age.value) < 18 || Number(form.elements.age.value) > 90) {
            notification.className = "notification";
            notification.classList.add("error");
            body.appendChild(notification);
            title.textContent = "Error";
